...
6. **A consent-service script enforces the read-only `DEFAULT`** (adopted; previously deferred). A small JavaScript consent service on each partitioned FHIR endpoint rejects write verbs (`CREATE`/`UPDATE`/`DELETE`/`PATCH` and write entries in a `transaction`) whose resolved request partition is `DEFAULT`, and otherwise proceeds. Curator accounts are exempt (identified by a superuser role / an explicit curator authority) so shared-data maintenance is unaffected. Reads of `DEFAULT` always proceed. This is the mechanism that makes points 2 and 3 hold, given the coarse partition permission. A reference implementation is at [`module-config/consent-default-readonly.js`](../../module-config/consent-default-readonly.js); wiring it and re-running the Phase 0 matrix with a DEFAULT-write-rejection case is a rollout step, gated on the same sign-off and rehearsal as the rest of the rollout.

   **Amended 2026-08-14 on implementation.** Two corrections to the above. The script is wired on the **FHIR REST Endpoint** module, not the persistence module as originally written: package registry seeding and IG installs write into `DEFAULT` below the REST layer, so a persistence mount would sit in the node's startup path and a script error there means the node never becomes healthy. And the reference implementation did not match Smile CDR's actual consent API (wrong callback arity, two callbacks misnamed, a message passed to a `reject()` that takes none), so it would have failed on every request had it been deployed as written. It ships in an observe mode that logs rather than rejects, so the parts that cannot be verified off the node get verified before they can block anything. Procedure, matrix and rollback: [consent-service-rollout.md](../consent-service-rollout.md).
...
- **Interim state (2026-07-16):** ahead of the consent service being wired and deployed, the `platypus-demo-patient` user on `ereq` was granted `FHIR_ACCESS_PARTITION_NAME: DEFAULT` directly (coarse — currently also permits `DEFAULT` writes) so SDC form-return writeback could be demonstrated. This is a demo-only stopgap to be normalised once the consent service lands (the grant stays; the write exposure is then closed centrally).
  ...
  **Update 2026-08-14.** That account now lives on `aucore` (`ereq` is decommissioned) and is seeded from `module-config/platypus-users.json` as `read-write` on `PLATYPUS,DEFAULT`. `connectathon-user-05` and `connectathon-user-06` are `read-write` with no tenant set, which resolves to `DEFAULT`; whether they still exist on the server is unconfirmed. The write exposure closes when the consent service moves from observe to enforcing, not when it is merged. Until then what actually prevents these accounts writing the curated dataset is that the clients they authenticate through hold read-only scopes.
